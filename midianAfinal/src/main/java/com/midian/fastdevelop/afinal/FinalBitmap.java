...
import com.midian.fastdevelop.afinal.bitmap.core.BitmapCache;
import com.midian.fastdevelop.afinal.bitmap.core.BitmapDisplayConfig;
import com.midian.fastdevelop.afinal.bitmap.core.BitmapProcess;
import com.midian.fastdevelop.afinal.bitmap.display.Displayer;
import com.midian.fastdevelop.afinal.bitmap.display.SimpleDisplayer;
import com.midian.fastdevelop.afinal.bitmap.download.Downloader;
import com.midian.fastdevelop.afinal.bitmap.download.SimpleDownloader;
import com.midian.fastdevelop.afinal.core.AsyncTask;
import com.midian.fastdevelop.afinal.utils.Utils;
...
public class FinalBitmap {
	...
	public void display(View imageView, String uri, int viewWidth) {
		doDisplay(imageView, uri, null, viewWidth);
	}

	private void doDisplay(View imageView, String uri, BitmapDisplayConfig displayConfig, Integer viewWidth) {
		if (!mInit) {
			init();
		}
		...
		if (bitmap != null) {
			...
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(imageView, displayConfig, null);
			//设置默认图片
	...
	private void displayImageView(View imageView, int width, Bitmap bitmap) {
		ViewGroup.LayoutParams lp = imageView.getLayoutParams();
		lp.width = width;
		lp.height = (int) (width * (bitmap.getHeight() / Float.valueOf(bitmap.getWidth())));
		imageView.invalidate();
	}

	public void doDownloadPic(ImageView imageView, String uri, DownPicListener downPicListener) {
		if (!mInit) {
			init();
		}
		...
		if (bitmap != null) {
			downPicListener.complete(bitmap);

		} else if (checkImageTask(uri, imageView)) {
			final BitmapLoadAndDisplayTask task = new BitmapLoadAndDisplayTask(imageView, mConfig.defaultDisplayConfig, downPicListener);
			//设置默认图片
			final AsyncDrawable asyncDrawable = new AsyncDrawable(mContext.getResources(), bitmap, task);
	...
	private class BitmapLoadAndDisplayTask extends AsyncTask<Object, Void, Bitmap> {
		...
		private DownPicListener downPicListener = null;

		public BitmapLoadAndDisplayTask(View imageView, BitmapDisplayConfig config, DownPicListener downPicListener) {
			imageViewReference = new WeakReference<View>(imageView);
			displayConfig = config;
			this.downPicListener = downPicListener;
		}
