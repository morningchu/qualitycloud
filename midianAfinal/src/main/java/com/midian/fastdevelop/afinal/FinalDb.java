...
import com.midian.fastdevelop.afinal.db.sqlite.CursorUtils;
import com.midian.fastdevelop.afinal.db.sqlite.DbModel;
import com.midian.fastdevelop.afinal.db.sqlite.ManyToOneLazyLoader;
import com.midian.fastdevelop.afinal.db.sqlite.OneToManyLazyLoader;
import com.midian.fastdevelop.afinal.db.sqlite.SqlBuilder;
import com.midian.fastdevelop.afinal.db.sqlite.SqlInfo;
import com.midian.fastdevelop.afinal.db.table.KeyValue;
import com.midian.fastdevelop.afinal.db.table.ManyToOne;
import com.midian.fastdevelop.afinal.db.table.OneToMany;
import com.midian.fastdevelop.afinal.db.table.TableInfo;
import com.midian.fastdevelop.afinal.exception.DbException;
...
public class FinalDb {
	 ...
	 * @return
	 */
	public static FinalDb create(Context context, String targetDirectory, String dbName, boolean isDebug, int dbVersion,
				...
				for (OneToMany one : ones) {
					...
					if (isFind) {
						...
						String strWhere = "("
								...
								+ id
								+ ")"
								+ ((one.getOrderColumn() != null && one.getOrderColumn().length() > 0) ? " order by " + one.getOrderColumn()
										+ (one.isDesc() ? " desc" : " asc") : "");
	...
	public SQLiteDatabase getDB(Class<?> clazz){
	   checkTableExist(clazz);
	   return db;
	}
	...
	private SQLiteDatabase createDbFileOnSDCard(String sdcardPath, String dbfilename, int dbVersion, DbUpdateListener dbUpdateListener) {
		...
		if (!dbf.exists()) {
			try {
				if (dbf.createNewFile()) {
					retDb = SQLiteDatabase.openOrCreateDatabase(dbf, null);
				}
		...
		//目前只支持升级，不支持降级
		if (retDb.getVersion() < dbVersion) {
			if (retDb.isReadOnly()) {
				throw new SQLiteException("Can't upgrade read-only database from version " + retDb.getVersion() + " to " + dbVersion + ": ");
			}

			retDb.beginTransaction();
			try {
				if (dbUpdateListener != null) {
					dbUpdateListener.onUpgrade(retDb, retDb.getVersion(), dbVersion);
				}
				retDb.setVersion(dbVersion);
				retDb.setTransactionSuccessful();
			} finally {
				retDb.endTransaction();
			}
		}
		return retDb;
	}
